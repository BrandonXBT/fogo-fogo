...
async function submitToSheet(payload) {
  // Send without CORS: prefer sendBeacon, fallback to hidden form POST
  const params = new URLSearchParams({
    secret: SUBMIT_SECRET,
    discordName: payload.discordName || '',
    walletAddress: payload.walletAddress || '',
    // aliases for compatibility with different Apps Script handlers
  ...
  if (navigator.sendBeacon) {
    try { navigator.sendBeacon(SHEET_WEBHOOK_URL, params); } catch(e) {}
    return true;
  }

  return await new Promise((resolve) => {
    const name = "hiddenSubmit_" + Math.random().toString(36).slice(2);
    ...
    setTimeout(() => { try{form.remove(); iframe.remove();}catch(e){} resolve(true); }, 900);
  });
}
